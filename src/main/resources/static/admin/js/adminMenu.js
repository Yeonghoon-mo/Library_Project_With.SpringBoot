...
async function subMenuList() {
    ...
    let mainMenuId = new URLSearchParams(location.search).get('mainMenuId'); mainMenuId *= 1;
    let boardMenuNum = new URLSearchParams(location.search).get('boardMenuNum'); boardMenuNum *= 1;
    let boardType = new URLSearchParams(location.search).get('boardType'); boardType *= 1;
    let subTitleHtml = '';
